...
export class OrderItemsComponent implements OnInit {
  ...
  constructor(
    ...
    ngOnInit(): void {
      this.itemService.retriveAllProduit().subscribe((res : HttpResponse<IProducts[]>) => (this.itemList = res.body || []));
  ...
  updatePrice(ctrl:any):void {
    if (ctrl.selectedIndex === 0) {
      ...
      this.stock = this.verificationStock(this.itemList[ctrl.selectedIndex - 1]);
    }
    this.updateTotal();
  }

  verificationStock(product: IProducts): number{
  ...
  validateForm(formData: LignesVentes): any {
    ...
    if (formData.productsId === 0)
      ...
      this.isValid = false;
      else if (formData.quantite! > this.stock)
        this.isValid = false;
    return this.isValid;
  }
