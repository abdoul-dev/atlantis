...
public class VentesResource {
    ...
    @GetMapping("/ventes/date/{date}")
    public BigDecimal getVenteDujour(@PathVariable LocalDate date) {
        log.debug("obtenir la vente du jour");
        return ventesService.venteduJour(date);
    }

    @GetMapping("/ventes/genererPDF/date/{date}")
    public void exportVentes(@PathVariable LocalDate date, HttpServletResponse response) throws Exception {
      log.debug("obtenir la vente du jour");
      String filename = ventesService.exportToPDF(date);
