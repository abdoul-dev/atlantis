...
export class TypeProduitComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.typeProduitService
      ...
      .subscribe(
        (res: HttpResponse<ITypeProduit[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  ngOnInit(): void {
    ...
    this.registerChangeInTypeProduits();
  }
  ...
  registerChangeInTypeProduits(): void {
    this.eventSubscriber = this.eventManager.subscribe('typeProduitListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ITypeProduit[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/type-produit'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
